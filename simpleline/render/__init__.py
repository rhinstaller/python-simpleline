...
from simpleline.base import send_exception, ExitMainLoop, ExitAllMainLoops
...
class Renderer(object):
    ...
    def __init__(self, app, renderer_stack=None, width=80):
        ...
        :param app: Event loop class.
        :type app: simpleline.base.App
    ...
    def do_redraw(self):
        ...
        if top_screen.execute_loop:
            ...
            try:
                ...
                send_exception(self._app.queue_instance, sys.exc_info())
                return False
    ...
    def raw_input(self, prompt, hidden=False):
        ...
        self._input_thread = threading.Thread(target=self._thread_input, name="InputThread",
                                              args=(self._app.queue_instance, prompt, hidden))
        self._input_thread.daemon = True
        self._input_thread.start()
        event = self._app.process_events(return_at=hubQ.HUB_CODE_INPUT)
    ...
    def input(self, args, key):
        ...
        if not self._screen_stack.is_empty():
            try:
                ...
                send_exception(self._queue_instance, sys.exc_info())
                return False
        ...
        # global quit command
            ...
            if self._app.quit_screen:
                d = self._app.quit_screen(self, self._app.quit_message)
                self.switch_screen_modal(d)
                if d.answer:
                    self._app.application_quit_cb()
